...
SerialCommReceiver::SerialCommReceiver(uint8_t portSelected)
{
	...
	if (portSelected == 0)
	{
		SerialCommReceiver::sInstance[0] = this;
	}
	else if (portSelected == 1)
	{
		SerialCommReceiver::sInstance[1] = this;
	}
	else if (portSelected == 2)
	{
		SerialCommReceiver::sInstance[2] = this;
	}
	else if (portSelected == 3)
	{
		SerialCommReceiver::sInstance[3] = this;
	}
...
void SerialCommReceiver::interrupt_RX3_Handler(void)
{
	uint8_t readByte;
...
ISR(USART3_RX_vect)
{
	SerialCommReceiver::sInstance[3]->interrupt_RX3_Handler();
}
